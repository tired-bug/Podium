...
router.post('/connect', requireAuth, requireRole('admin', 'developer'), async (req: AuthRequest, res: Response) => {
  ...
  getDb().prepare(`
    ...
    (id, repo_url, branch, token, auto_deploy, last_commit_sha, last_commit_message, last_synced, status)
  ...
  const repoRow = getDb().prepare('SELECT * FROM github_repos WHERE id = ?').get(id);
  return res.status(201).json(repoRow);
});
...
router.post('/repos/:id/pull', requireAuth, async (req, res: Response) => {
  ...
  const match = repo.repo_url.match(/github\.com\/([^/]+)\/([^/]+?)(?:\.git)?\/?$/);
  ...
  try {
    ...
    const resp = await axios.get(
      `https://api.github.com/repos/${owner}/${repoName}/commits/${repo.branch}`,
      { headers }
    );
  ...
  getDb().prepare(`
    UPDATE github_repos
    SET last_commit_sha = ?, last_commit_message = ?, last_synced = datetime('now')
    WHERE id = ?
...
router.post('/webhook', (req, res: Response) => {
  ...
  const repo = getDb()
    .prepare('SELECT * FROM github_repos WHERE repo_url = ? AND branch = ?')
