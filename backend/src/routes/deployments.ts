...
function logToDb(deploymentId: string, level: string, message: string, stream = 'stdout') {
  try {
    getDb()
      .prepare(
        'INSERT INTO build_logs (deployment_id, level, message, stream) VALUES (?, ?, ?, ?)'
      )
...
function simulateDeployment(id: string, name: string) {
  ...
  db.prepare("UPDATE deployments SET status='building', updated_at=datetime('now') WHERE id=?").run(
    id
  );
  ...
  setTimeout(() => {
    try {
      db.prepare("UPDATE deployments SET status='running', updated_at=datetime('now') WHERE id=?").run(
        id
      );
      logToDb(
        id,
        'info',
        `[Demo] "${name}" is running in demo mode. Start Docker to deploy for real.`
      );
    } catch {}
  }, 6000);
}
...
function simulateStop(id: string, name: string) {
  ...
  setTimeout(() => {
    try {
      getDb()
        .prepare(
          "UPDATE deployments SET status='stopped', updated_at=datetime('now') WHERE id=?"
        )
        .run(id);
...
async function startDockerDeployment(dep: any): Promise<void> {
  ...
  getDb()
    .prepare("UPDATE deployments SET status='building', updated_at=datetime('now') WHERE id=?")
    .run(id);
  ...
  try {
    const envVars: string[] = (
      JSON.parse(dep.env_vars || '[]') as Array<{ key: string; value: string }>
    ).map((e) => `${e.key}=${e.value}`);
    ...
    const container = await docker.createContainer({
      ...
      HostConfig: {
        ...
        CpuQuota: Math.floor(parseFloat(dep.cpu_limit || '0.5') * 100000),
    ...
    getDb()
      .prepare(
        "UPDATE deployments SET status='running', container_id=?, updated_at=datetime('now') WHERE id=?"
      )
      .run(info.Id, id);
    ...
    getDb()
      .prepare("UPDATE deployments SET status='failed', updated_at=datetime('now') WHERE id=?")
      .run(id);
...
async function smartStart(dep: any) {
  ...
  if (hasDocker) await startDockerDeployment(dep);
  else simulateDeployment(dep.id, dep.name);
}
...
router.post('/', requireAuth, requireRole('admin', 'developer'), async (req: AuthRequest, res: Response) => {
  ...
  return res.status(201).json({
    ...dep,
    ports: JSON.parse(dep.ports),
    env_vars: JSON.parse(dep.env_vars),
  });
});
